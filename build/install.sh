...
Now you should access CLI script: $ exercise-mvc --help
Alternatively you can access CLI script via poetry run: $ poetry run exercise-mvc --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_EXERCISE_MVC_COMPLETE=source_bash exercise-mvc)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_EXERCISE_MVC_COMPLETE=source_zsh exercise-mvc)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_EXERCISE_MVC_COMPLETE=source_fish exercise-mvc)' >> ~/.config/fish/completions/exercise-mvc.fish
EOF