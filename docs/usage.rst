...
.. code-block:: bash
    ...
    $ exercise-mvc --help
    Usage: exercise-mvc [OPTIONS] COMMAND [ARGS]...

      Exercise-mvc CLI root.
    ...
    Commands:
      serve  exercise-mvc CLI serve command.
    $ exercise-mvc serve --help
    Usage: exercise-mvc serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name exercise-mvc -p 8000:8000 exercise-mvc:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/exercise-mvc:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  exercise-mvc
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``exercise_mvc.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean exercise-mvc image
      install          Install exercise-mvc with poetry
      metrics          Run exercise-mvc metrics checks
      unit-test        Run exercise-mvc unit tests
      integration-test  Run exercise-mvc integration tests
      docs             Build exercise-mvc documentation
...
* RedisClient ``exercise_mvc.app.utils.redis``
* AiohttpClient ``exercise_mvc.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from exercise_mvc.app.utils import RedisClient
...
.. literalinclude:: ../exercise_mvc/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``exercise_mvc.config`` submodule.
...
.. literalinclude:: ../exercise_mvc/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``exercise_mvc.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../exercise_mvc/app/router.py
    :language: python
